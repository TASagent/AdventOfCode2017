...
Console.WriteLine("Day 09 - Stream Processing");
...
class Group
{
    ...
    public int GetScore() => tier + children.Select(x => x.GetScore()).Sum();
    public int GetTotalGarbage() => garbageCharacters + children.Select(x => x.GetTotalGarbage()).Sum();
}
