...
Instructions[] instructions = File.ReadAllLines(inputFile)
    .Select(x => new Instructions(x, true))
    .ToArray();

int instrNum = 0;
int refSentCount = 0;

while (instructions[instrNum].Execute(ref instrNum, ref refSentCount, registers, null, null) == MachineState.Running)
{
...
instructions = File.ReadAllLines(inputFile)
    .Select(x => new Instructions(x, false))
    .ToArray();
...
public readonly struct Instructions
{
    ...
    private readonly bool oldBehavior;

    public Instructions(string line, bool oldBehavior)
    {
