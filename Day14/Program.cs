...
Console.WriteLine("Day 14 - Disk Defragmentation");
...
CircularList[] circularLists = new CircularList[128];
for (int i = 0; i < 128; i++)
{
...
byte[] Encode(int index, string input)
{
...
static IEnumerable<bool> GetStates(byte input)
{
    int filter = 1 << 7;
    for (int i = 0; i < 8; i++)
    {
...
static bool IsValid(int x, int y, bool[,] grid, Queue<(int, int)> pendingPoints)
{
